...
// TestCustomConfigPathUnderThePluginsRootBindsItsOwnSection closes the gap
// where a Definition that renames its section but stays under plugins.* was
// excluded from the section scan, so its own configuration was reported as
// belonging to no plugin.
func TestCustomConfigPathUnderThePluginsRootBindsItsOwnSection(t *testing.T) {
	t.Parallel()
	...
	sections, err := ConfigSections([]plugin.Bundle{plugin.BundleOf(definition)})
	require.NoError(t, err)
	paths := make([]string, len(sections))
	for index, section := range sections {
		paths[index] = section.Path
	}
	assert.Contains(t, paths, "plugins.actual-section",
		"the declared section is what makes a sibling typo beside it unowned rather than unnoticed")
	assert.NotContains(t, paths, "plugins.renamed",
		"the conventional name is not a fallback once ConfigPath is set")
}
