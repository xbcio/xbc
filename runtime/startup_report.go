...
// what grants that. A test in this package can build an *assembly.Container
// directly and never call newApp, which is exactly what startup_report_test.go does
...
func (a *App) report(order []*assembly.Instance, migrate bool) {
	l := a.log()
...
// it does not.
func capabilityTokens(inst *assembly.Instance) []string {
	p := inst.Plugin()
	var tokens []string
...
func depsAndProvidesSuffix(inst *assembly.Instance) string {
	var parts []string
...
// to test with.
func renderInstanceTable(order []*assembly.Instance) string {
...
func renderMigrationNotice(order []*assembly.Instance, migrate bool) string {
	if migrate {
		return ""
	}
	n := 0
