...
func TestOrphanConfigurationFailsStartupWhileADisabledSectionDoesNot(t *testing.T) {
	...
	assert.Equal(t, 1, code)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "plugins.knwon", "the failure names the path a reader would edit")
	assert.Contains(t, err.Error(), "plugins.known", "and names the declared section it was likely meant to be")
...
// TestUnownedConfigurationIsCaughtBesideACustomConfigPath is the end-to-end
// proof for the shape that used to escape entirely: a Definition that renames
// its section to a dotted ConfigPath owns a path no fixed scan knows about, so
// a typo beside it was claimed by neither the plugins scan nor strict bind.
func TestUnownedConfigurationIsCaughtBesideACustomConfigPath(t *testing.T) {
	definition := plugin.Define("renamed", func(plugin.BuildContext) (*runtimeTestValue, error) {
		return &runtimeTestValue{}, nil
	}, plugin.Options[*runtimeTestValue]{ConfigPath: "plugins.group.actual"})

	accepted := newRuntimeTestApp(definition)
	code, err := accepted.Execute(context.Background(), append([]string{"doctor"},
		runtimeTestConfigWith(t, time.Second, "plugins:\n  group:\n    actual:\n      enabled: true\n")...))
	require.NoError(t, err, "the section the Definition declares is owned, however deep it sits")
	assert.Equal(t, 0, code)

	typo := newRuntimeTestApp(definition)
	code, err = typo.Execute(context.Background(), append([]string{"doctor"},
		runtimeTestConfigWith(t, time.Second,
			"plugins:\n  group:\n    actual:\n      enabled: true\n    typo:\n      enabled: true\n")...))
	assert.Equal(t, 1, code)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "plugins.group.typo", "the failure names the full path, not just its root")
	assert.Contains(t, err.Error(), "plugins.group.actual", "and names the section declared beside it")
}
