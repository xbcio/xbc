...
func (a *App) bindLifecycleContexts(parent context.Context, order []*container.Instance) {
...
// leak it.
//
// The Context handed to Init is the one the container already built and bound
...
// drifting apart.
func (a *App) initAll(order []*container.Instance) error {
	for _, inst := range order {
		if a.stopRequested() {
...
func (a *App) migrateAll(order []*container.Instance) error {
	for _, inst := range order {
		if a.stopRequested() {
...
// shutdown.
func (a *App) startRunners(order []*container.Instance) error {
	for _, inst := range order {
		if a.stopRequested() {
...
// bound successfully.
func (a *App) openTraffic(order []*container.Instance) error {
	for _, inst := range order {
		if a.stopRequested() {
