...
// --- assembly helper -------------------------------------------------------
//
// Every renderer here takes *container.Instance / *container.Container
// values, so the tests need genuinely assembled Instances. assemble below
// builds a real *container.Container the same way internal/container's own
// helpers_test.go does for its package, which is what lets these tests
...
// *container.Container, mirroring internal/container/helpers_test.go's own
// fakeHost. It is built with a nil c and wired up right after container.New,
...
// assemble builds and assembles a *container.Container from defs, failing the
...
func assemble(t *testing.T, envData map[string]any, defs ...plugin.Definition) *container.Container {
	t.Helper()
...
func TestCapabilityTokensListsInLifecycleOrder(t *testing.T) {
	...
	var full, sparse *container.Instance
...
func TestRenderInstanceTableColumnWidthAdaptsToLongestLabel(t *testing.T) {
	...
	var short, long *container.Instance
...
// topology.Miss but must render capitalized, and every entry must carry the
