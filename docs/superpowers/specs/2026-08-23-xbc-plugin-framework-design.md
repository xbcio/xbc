...
> **本文 §10 的目录树及 `application.example.yml` 等文件名仅是历史提案，不描述当前仓库。当前布局以权威文档 §3 为准：根包仅保留公开门面，协议无关实现分别由根级 `runtime/`、`assembly/`、`cli/` 包拥有；可执行配置样例位于 `examples/quickstart/application.yml`。**
>
