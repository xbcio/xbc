...
Two different identities appear when asking "who holds this slot", and they answer different questions. `xbc.instance_id` names the process: derived from the hostname, the boot second and a random suffix when left empty, and settable per process as `XBC_INSTANCE_ID` -- see [`xbc.instance_id`](quickstart.md#configuration) for the derivation and the whitespace rule. The slot's *owner token* is generated by the lease backend and stored under the slot key; it identifies a claim, not a process.

The identity is what gets reported, because it is the one an operator can act on. `doctor`'s `holder` line and `Stats().Instance` both carry it, so a report names a process you can go and look at rather than a token. The token remains available per slot as `Stats().Held[].Owner`, and it is what turns a claim found by reading the store directly into this process's claim. One log line binds the two at acquisition, which is the only place both appear together:

```
INFO  placement: slot won  workload=ingest slot=0 key=xbc:workload:ingest:0 instance=host-7-1758091200-9f3c1a2b owner=4f2ab9c1…
```
