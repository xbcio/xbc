...
Readiness only reports what contributes to it: selecting the health capability alone yields an empty `checks` array. `redis.Bundle()` and `gorm.Bundle()` each select a readiness probe next to their client Definition, so every configured instance is probed once the health Bundle is also selected -- reported as `redis-health` and `gorm-health` for the default instance, or `redis-health/<instance>` for a named one. They need that second Definition because their primary value is a third-party type. `elasticsearch`, `asynq`, and `raft` own their primary type, so it carries the contract directly and each check is named after the producing identity: `elasticsearch[search]`, `asynq`, `raft`. An application component contributes the same way, by exporting `health.Contributor` from its own Definition. Checks inherit `plugins.health.timeout` unless the contributor sets a per-check timeout, and the neutral `plugins.health` section is separate from the HTTP-facing `plugins.health-http` section below.

Two settings change what readiness reports. `plugins.elasticsearch.<instance>.health_probe: false` removes that cluster from readiness entirely, matching what it already does to the startup probe. The `raft` check asks whether the cluster has a leader, not whether this node is the leader, so a follower stays ready; a node that never bootstrapped and has no peers reports down for as long as it cannot elect one. Some integrations deliberately contribute nothing: `outbox` depends only on a `gorm` database that `gorm-health` already probes, and `cron` has no remote dependency unless distributed mode is enabled, where a lost lock surfaces within seconds through its own logged acquisition and renewal failures.
