...
// Plugin aggregates the contributors injected by its Definition and answers
// probes programmatically. It names no transport: a protocol adapter reads
// Check and renders the Report for its own wire format.
type Plugin struct {
	cfg          Config
...
// means shutdown has begun, so readiness must stop reporting the instance as
// able to serve production work even while a transport drains what it accepted.
