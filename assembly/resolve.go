...
func (c *Container) resolve(insts []*Instance) ([]*Instance, []ordering.Miss, error) {
	...
	var misses []ordering.Miss
	for _, inst := range insts {
		for _, name := range inst.deps.After {
			targets, ok := byPluginKey[name]
			if !ok {
				misses = append(misses, ordering.Miss{Node: inst.ID(), Ref: name.String(), Dir: ordering.After})
				continue
			}
		...
		for _, name := range inst.deps.Before {
			targets, ok := byPluginKey[name]
			if !ok {
				misses = append(misses, ordering.Miss{Node: inst.ID(), Ref: name.String(), Dir: ordering.Before})
				continue
			}
			for _, target := range targets {
...
func resolveTypeDep(g *ordering.Graph, inst *Instance, dep plugin.Dep, produced map[registryKey]*Instance, allProducts []product, productsByInstance map[string][]product) string {
...
func resolveRef(g *ordering.Graph, inst *Instance, ref plugin.Ref, byKey map[plugin.Key][]*Instance) string {
	wantKey := ref.Key()
...
// diagnostics above -- it is the assembly package's own responsibility, not a mirror of
// anything in plugin (plugin no longer has an equivalent at all: gap 4 of
// the plugin/assembly split deleted plugin.deriveName and the
