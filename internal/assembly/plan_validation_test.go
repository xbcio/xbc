...
func TestOrphanConfigurationIsReportedOnlyForUnknownConventionalSections(t *testing.T) {
	t.Parallel()
	...
	_, err = planFor(t, map[string]any{"plugins": map[string]any{
		...
		"knwon": map[string]any{},
		"other": map[string]any{},
	}}, definition)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "xbc: plugins.knwon has configuration but no corresponding plugin")
	assert.Contains(t, err.Error(), "xbc: plugins.other has configuration but no corresponding plugin")
}
