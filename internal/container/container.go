...
type Options struct {
	...
	// built around. The caller (the root package's hostAdapter) is expected
...
// consumes its output -- that stays the root package's job (see its
...
// Assemble owns the complete container assembly sequence: expand the
...
// broken dependency.
func (c *Container) SoftMisses() []topology.Miss {
	out := make([]topology.Miss, len(c.softMisses))
...
// registry. This is the plugin.RuntimeHost method the root package's hostAdapter
