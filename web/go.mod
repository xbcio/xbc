module github.com/xbcio/xbc/web
...
// of the one thing web/go.mod must be able to prove, namely that an outside user
// can `go get` it. Once core is tagged, run `go mod tidy` here (with GOWORK=off)
// and the require line appears with the real version. Release order is fixed:
// core -> web -> examples.
require (
