...
// so that plugin.Context can reach the container's value registry and the
// app's task runtime without the plugin package importing either -- plugin
...
// the whole application. A plugin that did so would get at the container, the
// settings, the shutdown machinery and the stop channel -- exactly the
...
// ProvideValue and LookupValue forward straight to the container's value
// registry, which is the (type, instance) store shared by every Context
...
// InitializedPlugins forwards to the container, which refuses to answer
// until every plugin has finished Init. That refusal is what makes
